...
class LocationBlinker(MapMarker):
    ...
    def blink(self):
        ...
        location_coordinates = running_app.curr_journey_points  # Coordinates in the Journey (Numpy array)
        ...
        if self.lat and self.lon:
            ...
            try:
                ...
                if ([self.lat, self.lon] != location_coordinates[-1]).all(): # Check that first coordinate does not equal to the previously added
                    location_coordinates = np.concatenate([location_coordinates, [[self.lat, self.lon]]])
        ...
        self.anim.bind(on_complete=self.reset) # Bind a function after being on_complete
    ...
    def stop_blink(self):
        ...
        running_app.curr_journey_points = None  # Here we reset journey to be empty Numpy Array
