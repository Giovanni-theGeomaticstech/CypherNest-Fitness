...
class MapView(Widget):
    ...
    def load_visible_tiles(self):
        ...
        tile_x_first, tile_y_first, tile_x_last, tile_y_last, x_count, y_count = bbox_for_zoom(
            vx, vy, self.width, self.height, zoom
        )
        ...
        for tile in self._tiles_bg[:]:
            ...
            w = self.width / f
            h = self.height / f
            btile_x_first, btile_y_first, btile_x_last, btile_y_last, _, _ = bbox_for_zoom(
                vx / f, vy / f, w, h, tile.zoom
            )
