            ...
            if len(polygons.meshes) != 0:
                ...
                for vertices, indices in polygons.meshes:
                    instruction_group.add(
                        Mesh(
                            vertices=vertices, indices=indices, mode=str("triangle_fan")
                        )
                    )
    ...
    def draw_path(self, gc, path, transform, rgbFace=None):
        ...
        if _mpl_ge_2_0:
            polygons = path.to_polygons(
                transform, self.widget.width, self.widget.height, closed_only=False
            )
        else:
            polygons = path.to_polygons(
...
class NavigationToolbar2Kivy(NavigationToolbar2):
    ...
    def draw_rubberband(self, event, x0, y0, x1, y1):
        ...
        rect = [
            int(val)
            for val in (min(x0, x1) + self.canvas.x, min(y0, y1) + self.canvas.y, w, h)
        ]
        ...
        self.lastrect.add(Rectangle(pos=(rect[0], rect[1]), size=(rect[2], rect[3])))
...
class GraphicsContextKivy(GraphicsContextBase, object):
    ...
    _capd = {"butt": "square", "projecting": "square", "round": "round"}
    line = {}
...
class FigureCanvasKivy(FocusBehavior, Widget, FigureCanvasBase):
        ...
        if self.collide_point(*touch.pos):
            ...
            if "button" in touch.profile and touch.button in ("scrollup", "scrolldown"):
                ...
                self.button_press_event(
                    x, y, self.get_mouse_button(touch), dblclick=False, guiEvent=None
                )
    ...
    def print_png(self, filename, *args, **kwargs):
        ...
        texture = Texture.create(size=(w, h))
        texture.blit_buffer(
            bytes(fig.get_renderer().buffer_rgba()), colorfmt="rgba", bufferfmt="ubyte"
        )
        texture.flip_vertical()
        img = Image(texture)
        img.save(filename)
